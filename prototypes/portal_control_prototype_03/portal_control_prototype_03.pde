...
float videoScale = 1.6;

// monitor scale to fit MONITORS for display side by side in a dashboard
float monitorScale = .35;

float growFactor = 1.5; // for increasing the size of the feedback layer
...
///////////////
//Functions
void assign_monitor_background_colors(){
	// Create the font
...
void write_labels_on_monitors() {
	//assign text to monitors
	textAlign(RIGHT);
	text("mx1_MONITOR", float(width/2), float(30));
